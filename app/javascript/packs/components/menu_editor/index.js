...
class MenuEditor extends React.Component {
  ...
  componentWillUpdate(nextProps) {
    if (nextProps.userTrainingMenu.state !== null) {
      ...
      if (this.state.fade === fadeState[0])
        this.setState({ fade: fadeState[1] });
    }
  }
  ...
  renderWeekDayOfTrainingSelector(training) {
    ...
    const currentValue = tryGetWeekDay(
      this.state.weekDaysOfTraining,
      training.id
    );
    return <WeekdaySelector currentValue={currentValue} onChange={onChange} />;
  }
  ...
  renderTrainingCollection() {
    ...
    const collection = this.props.state.map(training => (
      <li {...props} key={key("list", training.id)}>
        <div
          style={{ display: "inline-block", width: "45%", marginTop: "30px" }}
        >
    ...
    return <ul className="collection">{collection}</ul>;
  }
    ...
    return (
      ...
      >
        ...
        <div style={{ height: "80%", textAlign: "center" }}>
          <p style={{ fontSize: "2em", marginBottom: "60px" }}>ようこそ薫陶へ</p>
